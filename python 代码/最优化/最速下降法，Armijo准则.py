...
# 定义最速下降法    
def gradient_descent(start_point, func, gradient, epsilon, c, gamma, max_iter=1000):        
    global Q, b         
...
# 输入参数   
x0 = np.array([2,3], dtype="float32").reshape([-1, 1])  # 使用numpy数组定义初始点  
xs, iter_num = gradient_descent(start_point=x0, func=func,gradient=gradient,c = c,gamma= gamma,epsilon=1e-20)  # 调用gradient descent函数并传入参数  
